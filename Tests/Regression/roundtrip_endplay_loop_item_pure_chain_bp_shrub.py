...
RESULT_PATH = os.path.join(OUTPUT_DIR, "roundtrip_endplay_loop_item_pure_chain_bp_shrub_result.json")
ORIGINAL_EXPORT_PATH = os.path.join(OUTPUT_DIR, "bp_shrub_endplay_loop_item_pure_chain_original_export.bplisp")
MODIFIED_INPUT_PATH = os.path.join(OUTPUT_DIR, "bp_shrub_endplay_loop_item_pure_chain_input.bplisp")
MODIFIED_EXPORT_PATH = os.path.join(OUTPUT_DIR, "bp_shrub_endplay_loop_item_pure_chain_export.bplisp")
RESTORED_EXPORT_PATH = os.path.join(OUTPUT_DIR, "bp_shrub_endplay_loop_item_pure_chain_restored_export.bplisp")

BP_PATH = "/Game/Blueprint/Interactable/Resources/BP_Shrub"
GRAPH_NAME = "EventGraph"
LOOP_VALUES = [f"PureChainLoop{i}" for i in range(4)]
NESTED_VALUES = ["Slot0", "Slot2", "Slot3"]
...
def build_endplay_event():
  ...
  (PrintString
    :instring
      (get-array-item
        (make-array "{NESTED_VALUES[0]}" ArrayElement "{NESTED_VALUES[1]}" "{NESTED_VALUES[2]}")
        ArrayIndex)
    :bprinttoscreen true
    :bprinttolog true
    :textcolor "(R=0,G=0.5,B=1,A=1)"
    :duration 2
    :key "None"))'''
...
def validate_modified_export(export_text):
    ...
    report["modified_contains_get_array_item"] = "get-array-item" in export_text
    report["modified_contains_array_element_ref"] = "Array Element" in export_text
    report["modified_contains_array_index_ref"] = "Array Index" in export_text
    ...
    report["modified_nested_value_hits"] = {value: (f'"{value}"' in export_text) for value in NESTED_VALUES}
    report["modified_nested_value_count"] = sum(1 for hit in report["modified_nested_value_hits"].values() if hit)
    report["modified_out_decl_string"] = ":out (Array Element string)" in export_text
    report["modified_out_decl_int"] = ":out (Array Index int)" in export_text
    ...
    if not report["modified_contains_get_array_item"]:
        raise RuntimeError("回导出未发现 get-array-item")
    if not report["modified_contains_array_element_ref"]:
        raise RuntimeError("回导出未保留 Array Element 符号引用")
    if not report["modified_contains_array_index_ref"]:
        raise RuntimeError("回导出未保留 Array Index 符号引用")
    if report["modified_loop_value_count"] != len(LOOP_VALUES):
        raise RuntimeError("回导出未完整保留 ForEachLoop 测试数组字符串值")
    if report["modified_nested_value_count"] != len(NESTED_VALUES):
        raise RuntimeError("回导出未完整保留嵌套 make-array 的固定字符串值")
    if not report["modified_out_decl_string"]:
        raise RuntimeError("回导出未保留 Array Element 的 string 类型声明")
    if not report["modified_out_decl_int"]:
        raise RuntimeError("回导出未保留 Array Index 的 int 类型声明")
...
def validate_restored_export(export_text):
    ...
    report["restored_has_get_array_item"] = "get-array-item" in export_text
    report["restored_has_loop_values"] = any(f'"{value}"' in export_text for value in LOOP_VALUES)
    report["restored_has_nested_values"] = any(f'"{value}"' in export_text for value in NESTED_VALUES)
    report["restored_has_array_element_ref"] = "Array Element" in export_text
    report["restored_has_array_index_ref"] = "Array Index" in export_text
    ...
    if report["restored_has_get_array_item"]:
        raise RuntimeError("恢复后仍残留 get-array-item")
    if report["restored_has_loop_values"]:
        raise RuntimeError("恢复后仍残留 ForEachLoop 测试数组值")
    if report["restored_has_nested_values"]:
        raise RuntimeError("恢复后仍残留嵌套 make-array 固定字符串值")
    if report["restored_has_array_element_ref"]:
        raise RuntimeError("恢复后仍残留 Array Element 引用")
    if report["restored_has_array_index_ref"]:
        raise RuntimeError("恢复后仍残留 Array Index 引用")
...
def main():
    ...
    log("BP_Shrub EndPlay + ForEachLoop + 嵌套 pure chain 片段语法校验通过")
    ...
    try:
        ...
        log(f"已生成 BP_Shrub 嵌套 pure chain 测试 DSL: {MODIFIED_INPUT_PATH}")
        ...
        log("BP_Shrub 完整嵌套 pure chain 测试 DSL 校验通过")
        ...
        log(f"已保存 BP_Shrub 嵌套 pure chain 回导出 DSL: {MODIFIED_EXPORT_PATH}")
        validate_modified_export(modified_export)
        log("已确认 BP_Shrub 回导出保留 ForEachLoop、Array Element / Array Index 与嵌套 pure chain")
    finally:
