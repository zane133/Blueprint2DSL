...
RESULT_PATH = os.path.join(OUTPUT_DIR, "roundtrip_endplay_foreach_bp_stone_result.json")
ORIGINAL_EXPORT_PATH = os.path.join(OUTPUT_DIR, "bp_stone_endplay_foreach_original_export.bplisp")
MODIFIED_INPUT_PATH = os.path.join(OUTPUT_DIR, "bp_stone_endplay_foreach_input.bplisp")
MODIFIED_EXPORT_PATH = os.path.join(OUTPUT_DIR, "bp_stone_endplay_foreach_export.bplisp")
RESTORED_EXPORT_PATH = os.path.join(OUTPUT_DIR, "bp_stone_endplay_foreach_restored_export.bplisp")

BP_PATH = "/Game/Blueprint/Interactable/Resources/BP_Stone"
GRAPH_NAME = "EventGraph"
LOOP_VALUES = [f"Loop{i}" for i in range(10)]
...
def validate_modified_export(export_text):
    ...
    report["modified_contains_helloworld"] = '"helloworld"' in export_text
    report["modified_contains_foreach_call_macro"] = "ForEachLoop" in export_text
    report["modified_contains_foreach_fallback"] = "for-each-loop" in export_text
    ...
    if not report["modified_contains_helloworld"]:
        raise RuntimeError("回导出未发现 helloworld")
    if not (report["modified_contains_foreach_call_macro"] or report["modified_contains_foreach_fallback"]):
        raise RuntimeError("回导出未发现 ForEachLoop / for-each-loop")
    ...
    if report["modified_loop_value_count"] != len(LOOP_VALUES):
        raise RuntimeError("回导出未完整保留 10 个循环数组字符串值")
    if report["modified_make_array_nil_count"] != 0:
        raise RuntimeError("回导出中的 make-array 仍出现 nil，说明字符串元素值没有保真")
...
def validate_restored_export(export_text):
    ...
    report["restored_has_helloworld"] = '"helloworld"' in export_text
    ...
    if report["restored_has_helloworld"]:
        raise RuntimeError("恢复后仍残留 helloworld")
    if report["restored_has_loop_values"]:
        raise RuntimeError("恢复后仍残留测试数组元素")
...
def main():
    ...
    try:
        ...
        log(f"已保存测试回导出 DSL: {MODIFIED_EXPORT_PATH}")
        validate_modified_export(modified_export)
        log("已确认 BP_Stone 回导出保留 EndPlay、ForEachLoop、make-array 与 helloworld")
    finally:
        if original_text and modified_applied:
            ...
            log(f"已保存恢复后回导出 DSL: {RESTORED_EXPORT_PATH}")
            validate_restored_export(restored_export)
            log("已确认 BP_Stone 已恢复到测试前状态")
